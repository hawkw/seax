...
impl State {
    ...
    pub fn eval(self,
        ...
        match self.control.pop().unwrap() {
            ...
            (InstCell(DUM), new_control) => State {
                stack: self.stack,
...
/// TODO: add (optional?) parameters for stdin and stdout
#[stable(feature="vm_core",since="0.2.0")]
pub fn eval_with(program: List<SVMCell>,
                 inp: &mut io::Read,
                 outp: &mut io::Write,
                 debug: bool)
                -> List<SVMCell> {
    ...
    while {
        ...
        machine = machine.eval(inp, outp, debug) // continue evaling
    };
    machine.stack
}
