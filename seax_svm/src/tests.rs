...
fn test_empty_eval_fail() {
    State::new().eval(&mut io::stdin(), &mut io::stdout());
}
...
fn test_ld_empty_env_fail() {
    State {
        ...
        dump:       Stack::empty(),
    }.eval(&mut io::stdin(), &mut io::stdout());
}
...
fn test_eval_nil () {
    ...
    state = state.eval(&mut io::stdin(), &mut io::stdout());
...
fn test_eval_null() {
    ...
    assert_eq!(
        State {
            ...
            dump: Stack::empty(),
        }.eval(&mut io::stdin(), &mut io::stdout()).stack.peek(),
